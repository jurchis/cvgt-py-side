...
def segment_image(image, file_path):
    
    # load the class label names
    #CLASSES = open(args["classes"]).read().strip().split("\n")
    CLASSES = open("enet-cityscapes/enet-classes.txt").read().strip().split("\n")
    
    # if a colors file was supplied, load it from 
    '''
    if args["colors"]:
    ...
    image_extension = file_path.split("/")[-1].split('.')[-1]
    
    # Save input image 
    cv2.imwrite("input/" + image_name + "." + image_extension, image) 
    # Save ouput image
    cv2.imwrite("output/" + image_name +"_out." + image_extension, output) 
    
    # show the input and output images
    cv2.imshow("Legend", legend)
    cv2.imshow("Input", image)
    cv2.imshow("Output", output)
    while (cv2.getWindowProperty('Legend',cv2.WND_PROP_VISIBLE) >= 1) \
        and (cv2.getWindowProperty('Input',cv2.WND_PROP_VISIBLE) >= 1)\
        and (cv2.getWindowProperty('Output',cv2.WND_PROP_VISIBLE) >= 1):
        cv2.waitKey(1)
